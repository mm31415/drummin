...
const drawCircles = () => {
  for(let i = 0; i < circlesQueue.length; i++) {
    ...
    if (circlesQueue[i].x >= 545) {
      circlesQueue[i].fill = "transparent";
      circlesQueue[i].stroke = "transparent";
    }
...
const setCurrentInstrument = (idx) => {
  if (circlesQueue[idx].y === 37.5 + 35 * 7 && circlesQueue[idx].fill !== "transparent") {
    currentBass = (currentBass > -1) ? currentBass : idx;
  }
};
...
module.exports = {
  ...
  resetBoardValue: resetBoardValue
};
...
const DrawGame = __webpack_require__(/*! ./game.js */ "./lib/game.js");
